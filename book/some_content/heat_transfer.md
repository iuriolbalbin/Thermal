...
Φ_total (T)= ∫▒〖ϵ_λ Φ_B (λ,T)  〗 dλ
An effective material emissivity can be determined for a material:
Φ_total (T)= ∫▒〖ϵ_λ Φ_B (λ,T)  〗 dλ≡ϵ_eff ∫▒〖Φ_B (λ,T)  〗 dλ=ϵ_eff  σT^4=ϵ σT^4
Where:
ϵ_eff=(∫▒〖ϵ_λ Φ_B (λ,T)  〗 dλ)/(∫▒〖Φ_B (λ,T)  〗 dλ)
...
Figure 5: Relation between absorptivity, reflectivity and transmissivity. Note that emissivity is not included in this relation, as it is the radiation generated by the body itself.
...
Figure 6: A graph showing the (hemispherical) reflectance of Aeroglaze A276 White paint as function of the wavelength. Source: Kauder, 2005
...
Figure 7: A theoretical example of an absorption spectrum (in green). I  blue a normalised solar spectrum is shown, in orange an example of an IR spectrum. The solar absorption α will be 0.2, while the IR emission coefficient will be 0.6
In general, optical properties can be divided into 4 categories seen in Figure 8. 
...
B_ij  = F_ij   ε_j  +  ∑_N▒〖(1 - ε_k)  F_ik   B_kj 〗
...
ε_i   A_i   B_ij  = ε_j   A_j   B_ji  = R_ij
